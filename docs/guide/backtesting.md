...
!!! note "# Trades vs # Positions"
    `# Trades` counts every closed `Trade` row, including partial closes: a
    position closed in two partial closes produces two rows there. `# Positions`
    (and `# Positions Long` / `# Positions Short`) count distinct positions
    instead - partial closes of the same position are merged into one - so
    a strategy that scales out of its positions does not look like it trades
    far more often than it actually opens/closes positions. `bt.stats.positions`
    exposes the underlying per-position `DataFrame` (entry/exit, summed size and
    PnL, and `n_partials`), while `bt.stats.trades` keeps the row-per-close view.
