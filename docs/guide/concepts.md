...
chart. `# Trades` counts every closed trade row (partial closes included),
while `# Positions` / `# Positions Long` / `# Positions Short` count distinct
positions - partial closes of the same position merged into one - see
[Metrics](backtesting.md#metrics). To test robustness, `bt.montecarlo(...)`
runs a Monte Carlo analysis - see [Monte Carlo robustness](robustness.md).
