...
def _render_overlay_indicators(fig, indicators: list, interval_ms: int, theme: dict = None,
                               ...
                               lazy_zoom_range: int = None,
                               hide_on_pan_sink: list = None) -> list:
    """
    ...
    for meta in indicators:
        ...
        if not _is_geometric_renderer(meta.renderer):
            ...
            render_indicator(fig, srcs, meta, interval_ms=interval_ms,
                             hide_on_pan_sink=hide_on_pan_sink, x_range=lazy_x_range)
...
def _build_equity_panel(config, theme, stats, all_figs):
    ...
    return fig_eq, eq_source, baseline, trailing_source
    ...
    for meta in indicators:
        ...
        if not is_geom:
            ...
            render_indicator(fig_ind, srcs, meta, interval_ms=interval_ms,
                             hide_on_pan_sink=hide_on_pan_sink, x_range=x_range)
            if meta.autoscale:
...
def plot(bt, config: PlotConfig | None = None, **kwargs) -> None:
    ...
    **kwargs : any PlotConfig field as keyword argument, alternative
    ...
    fig_eq, eq_source, baseline, eq_trailing_source = _build_equity_panel(
        config, theme, stats, all_figs
    )
    ...
    if config.equity_mode != "none" and stats is not None:
        _configure_autoscale_equity(
            fig_eq, eq_source, x_range, baseline,
            trailing_source=eq_trailing_source,
        )
    ...
    # ── Superimpose: translucent higher-TF candle bodies behind the OHLC ──
    # Resolved AFTER ohlc_array/interval_ms are finalized (post footprint/
    # resample/max_candles) so the auto-selection table sees the real chart
    # interval. Excluded from the Y-autoscale (not fed to _configure_
    # autoscale_ohlc below) and left out of the derender registry - its
    # geometry is a handful of bars, cheap to keep rendered during pan/zoom.
    _superimpose_interval = _resolve_superimpose_interval(
        interval_ms, len(ohlc_array), config.superimpose
    )
    if _superimpose_interval is not None:
        from tradetropy.plotting._util import _interval_label
        _si_array, _si_interval = _resample_ohlc(
            ohlc_array, interval_ms, _superimpose_interval
        )
        _si_source = build_ohlc_source(_si_array, _si_interval)
        render_superimpose(fig_ohlc, _si_source, theme, _interval_label(_si_interval))
    ...
    # Pan/zoom blanking targets kept SEPARATE from the lazy-label list. Marker
    # series (NBS triangles, ...) blank only while the user drags/wheels
    # (``_configure_derender``); they must NOT be governed by
    # ``_configure_lazy_labels``, whose zoom-threshold hide is meant for TEXT
    # labels only. Sharing one list made the markers vanish permanently when
    # zoomed out (lazy_labels re-hid them on every range change, overriding the
    # PanEnd restore) and, because a marker series owns a legend entry, toggling
    # its visibility on every range tick made that legend entry flicker.
    hide_on_pan_entries: list = []
    ...
    _heatmap_autoscale_sources, _series_autoscale_sources = _render_overlay_indicators(
        ...
        lazy_zoom_range=_lazy_zoom, hide_on_pan_sink=hide_on_pan_entries,
    )
    ...
    ind_figs = _build_indicator_panels(
        ...
        hide_on_pan_sink=hide_on_pan_entries,
    )
    ...
    _configure_crosshair(all_figs, theme, tick_size=(price_tick or fp_tick_size),
                         ohlc_fig=fig_ohlc)
    # Derender blanks BOTH text labels and marker series while panning/zooming;
    # lazy-labels (above) only governs the text labels' zoom-threshold hide.
    _configure_derender(
        all_figs, lazy_label_entries + hide_on_pan_entries,
        enabled=config.derender_on_pan,
    )
    ...
    # ── Manual header / footer notes ─────────────────────────────────────
    # Header sits above everything (including the stats bar); footer sits
    # below the last panel. Both are opt-in (None -> nothing rendered).
    if config.header_text:
        all_figs.insert(0, _build_note_div(config.header_text, theme, position="header"))
    if config.footer_text:
        all_figs.append(_build_note_div(config.footer_text, theme, position="footer"))
