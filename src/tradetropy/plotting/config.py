...
class IndicatorPlotMeta:
    ...
    # ── Pan/zoom derender override ───────────────────────────────────────────
    # None -> resolved per dimension from renderer type (see
    # ta.base.resolve_hide_on_pan). Explicit True/False overrides that default
    # regardless of renderer.
    hide_on_pan: bool | None = None
...
class PlotConfig:
    ...
    show_price_tag       : DEPRECATED, removed. The floating price box that
                           used to follow the crosshair has been removed
                           entirely (the native crosshair already shows the
                           value on the axis). This kwarg is still ACCEPTED
                           for backwards compatibility and silently ignored,
                           emitting a DeprecationWarning; it will be removed
                           in a future version.
    ...
    title                : Browser tab / window title (also used as the
                           Bokeh Server document title when output="server").
                           Default "Backtest".
    ...
    header_text          : Optional manual note rendered as a Div ABOVE the
                           whole layout (above the stats bar, if any). The
                           string is injected as raw HTML (not escaped), so it
                           can hold basic markup (<b>, <a>, ...) -- do not feed
                           it untrusted/external text. None (default) renders
                           nothing extra.
    footer_text          : Same as header_text but rendered as a Div BELOW the
                           whole layout (after every panel). None (default)
                           renders nothing extra.
    superimpose          : Overlay translucent, wick-less candle BODIES of a
                           higher timeframe behind the main OHLC (backtesting.
                           py-style), so the higher-TF trend/structure reads
                           through the finer candles. Accepts:
                             - False (default): disabled.
                             - True: auto-selects a higher TF from the chart's
                               own interval (sub-minute/minutes -> 1h, hourly
                               -> 1D, daily -> 1W). If no candidate is
                               strictly higher (or resampling would yield
                               fewer than ~3 bars), superimpose is silently
                               skipped with a warning.
                             - str (e.g. "4h", "1D"): explicit target
                               timeframe, parsed via parse_timeframe(). Must
                               be strictly greater than the chart's own
                               interval; otherwise a warning is emitted and
                               superimpose is skipped.
                           Drawn as quads (open/close body only, no high/low
                           wick), excluded from the OHLC Y-autoscale and from
                           the pan/zoom derender registry (its geometry is
                           already a handful of bars, cheap to keep rendered).
                           Backtest-plot only; not available in the live chart.
    derender_on_pan      : Temporarily hide heavy overlay glyphs (scatter
                           markers, text labels) while the user is actively
                           dragging or wheel-zooming the chart, restoring them
                           once the interaction settles. Keeps pan/zoom fluid
                           on charts with many marker-based indicators (e.g.
                           NBS triangles). Bar/line/step renderers are not
                           affected - see IndicatorPlotConfig.hide_on_pan for
                           the per-indicator override. Default True; set to
                           False to disable the whole mechanism (e.g. while
                           visually debugging a chart).
    """
    ...
    show_price_tag: bool = False  # DEPRECATED: accepted, ignored, warns.
    ...
    superimpose: bool | str = False
    derender_on_pan: bool = True

    def __post_init__(self):
        if self.show_price_tag:
            import warnings
            warnings.warn(
                "PlotConfig.show_price_tag is deprecated and has no effect: "
                "the floating price tag has been removed from the chart. "
                "This kwarg will be removed in a future version.",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        if isinstance(self.superimpose, str):
            from tradetropy.core.constants import parse_timeframe
            self.superimpose = parse_timeframe(self.superimpose)
