...
def _configure_autoscale_equity(
    fig, source, x_range, baseline: float, trailing_source=None
) -> None:
    """
    Autoscale the equity panel when the X range changes.
    Respects the baseline (0 for return, initial_balance for balance).

    When a trailing-drawdown line is drawn (``trailing_source``), its extremes
    are folded into the Y-range so the trailing DD floor never falls off-screen
    on pan/zoom - matching the initial framing done in ``render_equity``.
    """
    from bokeh.models import CustomJS

    cb = CustomJS(
        args=dict(
            source=source, y_range=fig.y_range, x_range=x_range,
            baseline=baseline, tdd_source=trailing_source,
        ),
...
def _configure_derender(figs: list, entries: list, enabled: bool = True) -> None:
    """
    Wire the global pan/zoom derender for heavy overlay glyphs.

    Temporarily hides the collected renderers (markers, text labels - see
    ``derender.js`` for the policy on what should and should not be covered)
    while the user drags (PanStart/PanEnd) or wheel-zooms (debounced, since
    MouseWheel has no "end" event) any of the given figures, restoring them
    once the interaction settles. Every figure shares the SAME renderer/group
    lists so a drag on any panel (OHLC, an indicator sub-panel, ...) hides
    every registered glyph across the whole chart, keeping the interaction
    consistent regardless of which panel the user is dragging.

    Args:
        figs (list): Every figure whose PanStart/PanEnd/MouseWheel events
            should trigger the derender (typically all panels sharing the
            time axis, same population as ``_configure_crosshair``).
        entries (list): ``(renderer, group_glyph)`` tuples; ``group_glyph``
            (or None) gates the renderer's restored visibility on its legend
            state, same contract as ``_configure_lazy_labels``.
        enabled (bool): When False, nothing is wired (PlotConfig.
            derender_on_pan == False) - useful to fully disable the mechanism
            while visually debugging a chart.
    """
    from bokeh.events import PanStart, PanEnd, MouseWheel
    from bokeh.models import CustomJS

    if not enabled or not entries:
        return

    renderers = [r for r, _ in entries]
    groups = [g for _, g in entries]

    start_cb = CustomJS(
        args=dict(renderers=renderers, groups=groups, kind="start"),
        code=(_JS_DIR / "derender.js").read_text(),
    )
    end_cb = CustomJS(
        args=dict(renderers=renderers, groups=groups, kind="end"),
        code=(_JS_DIR / "derender.js").read_text(),
    )
    wheel_cb = CustomJS(
        args=dict(renderers=renderers, groups=groups, kind="wheel"),
        code=(_JS_DIR / "derender.js").read_text(),
    )

    for fig in figs:
        if not hasattr(fig, "js_on_event"):
            continue
        fig.js_on_event(PanStart, start_cb)
        fig.js_on_event(PanEnd, end_cb)
        fig.js_on_event(MouseWheel, wheel_cb)
...
def _configure_crosshair(
    figs: list, theme: dict, tick_size: float = 0.0,
    ohlc_fig=None,
) -> None:
    """
    Wire the shared crosshair across all panels.
    ...
    Args:
        ...
        theme (dict): Plot theme (crosshair color).
        tick_size (float): Unused (kept for call-site compatibility with
            callers that still compute a price tick).
        ohlc_fig: Unused (kept for call-site compatibility).
    """
...
def _show_layout(layout, config: PlotConfig, theme: dict) -> None:
    ...
    if config.output == "notebook":
        ...
        output_file(config.filename, title=config.title)
        show(full_layout)
...
# Ratio-type metrics: always rendered with 2 decimals in the stats bar, even
# when abs(value) >= 1 (the generic fallback in _build_stats_div would
# otherwise round Sharpe/Sortino/Calmar/PF/SQN to whole numbers, e.g. hiding
# 2.34 as "2").
...
def _build_note_div(text: str, theme: dict, position: str = "header") -> object:
    """
    Build a Div rendering a manual, user-supplied note above/below the chart.

    Used for ``PlotConfig.header_text`` / ``footer_text``: a free-form
    annotation (title, disclaimer, run notes, ...) the caller wants attached
    to the layout without touching any panel. The text is injected as raw
    HTML (same policy as ``_build_stats_div``'s cells) so basic markup
    (``<b>``, ``<a>``, line breaks via ``<br>``) works; do not feed it
    content from an untrusted/external source since it is not escaped.

    Args:
        text (str): Note content (raw HTML allowed).
        theme (dict): Plot theme (background/text/border colors).
        position (str): "header" (border on the bottom edge, sits above the
            rest of the layout) or "footer" (border on the top edge, sits
            below). Purely cosmetic - the caller controls actual placement
            in the figure list.

    Returns:
        Div: Bokeh Div ready to insert into the figure list passed to
             ``_build_layout()``.
    """
    from bokeh.models import Div
    ...
    border_side = "border-bottom" if position == "header" else "border-top"
    ...
    <div class="plot-note">{text}</div>
    """

    return Div(text=html, sizing_mode="stretch_width", styles={"min-height": "0"})
            ...
            if val is not None and val != "":
                if isinstance(val, float):
                    if pd.isna(val):
                        ...
                        # Ratios (Sharpe, Sortino, Calmar, PF, SQN) always show
                        # 2 decimals -- the generic fallback below drops them
                        # for any value >= 1, hiding e.g. Sharpe 2.34 as "2".
                        val_str = f"{val:.2f}"
                    else:
