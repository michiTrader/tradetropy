...
# Epoch-ms sanity bound (year 2001). ``ts_band_indices`` is also reused by some
# indicators to carry plain extra data bands (volume, notional, side...), so a
# band only counts as a real timestamp band when its values look like epoch ms.
# ``build_indicator_source`` applies the same test when it takes the x position
# from a ts band, so both sides agree on what a ts band is.
_TS_BAND_MS_THRESHOLD = 1_000_000_000_000


def _has_real_ts_bands(indicator, bands: np.ndarray) -> bool:
    """
    Whether ``indicator`` emits per-point event timestamps (a real ts band).

    An indicator that carries the timestamp of the bar each value belongs to
    (ConfirmedPivot, NBS, ZigZag, HHLL...) is an EVENT series: a value exists
    only on the bars where something happened, and the plot draws it at its own
    timestamp. Such a series must never be forward-filled onto another
    timeframe's bar index (see :func:`_align_events_to_index`).

    Args:
        indicator: The indicator instance from an indicator_def.
        bands (np.ndarray): Its raw output bands ``[K x N]``.

    Returns:
        bool: True when at least one declared ts band holds epoch-ms values.
    """
    for i in (getattr(indicator, "ts_band_indices", None) or ()):
        if i >= len(bands):
            continue
        finite = bands[i][np.isfinite(bands[i])]
        if finite.size and float(finite[0]) > _TS_BAND_MS_THRESHOLD:
            return True
    return False


def _align_events_to_index(
    values: np.ndarray,
    src_ts_ms: np.ndarray,
    target_ts_ms: np.ndarray,
) -> np.ndarray:
    """
    Map a sparse EVENT band onto the chart's bar index without forward-filling.

    Used for indicators mounted on a secondary (e.g. higher) timeframe whose
    output is a set of discrete events - pivots, NBS tags, zigzag legs - rather
    than a continuous series. Forward-filling those (what
    :func:`_resample_indicator_to_index` does, correctly, for a continuous
    series such as an HTF moving average) repeats every event on all the
    primary bars until the next one; because the marker's x comes from the
    indicator's ts band, all those copies land on the SAME pixel, so one 5m
    pivot became ~5 (or hundreds, for a rare band) identical stacked glyphs and
    the HoverTool listed the same value once per copy.

    Each finite sample is placed on the primary bar that contains its source
    bar (last target ts <= source ts); every other slot stays NaN, so the band
    keeps exactly one entry per real event. When several source bars fall in the
    same primary bar (secondary timeframe finer than the chart's) the last one
    wins, matching the keep-last policy of the resample path.

    Args:
        values (np.ndarray): One raw band ``[N_src]`` (price or ts band).
        src_ts_ms (np.ndarray): Open timestamp (epoch ms) of each source bar.
        target_ts_ms (np.ndarray): Open timestamp of each chart bar (sorted).

    Returns:
        np.ndarray: ``[N_target]`` float64 band, NaN outside the event bars.
    """
    out = np.full(len(target_ts_ms), np.nan, dtype=np.float64)
    vals = np.asarray(values, dtype=np.float64)
    src = np.asarray(src_ts_ms, dtype=np.float64)
    n = min(len(vals), len(src))
    if n == 0 or len(out) == 0:
        return out
    vals, src = vals[:n], src[:n]

    finite = np.isfinite(vals) & np.isfinite(src)
    if not finite.any():
        return out

    pos = np.searchsorted(np.asarray(target_ts_ms, dtype=np.float64),
                          src[finite], side="right") - 1
    keep = pos >= 0
    if keep.any():
        out[pos[keep]] = vals[finite][keep]
    return out
                ...
                if defn.get("multi_band") and defn.get("col_names"):
                    ...
                    if band_arrays:
                        ...
                        if is_primary:
                            ...
                            if _has_real_ts_bands(defn["indicator"], raw_multi):
                                # Event series (pivots/NBS/zigzag on another
                                # timeframe): place each event on its own bar,
                                # never forward-fill it over the chart's bars.
                                values = np.vstack([
                                    _align_events_to_index(
                                        row, src_ts, ohlc_timestamps_ms
                                    )
...
# Auto-selection table for PlotConfig.superimpose=True: (max own interval_ms
# this rule applies to, target interval_ms). Mirrors the intuitive backtesting
# .py-style jump ("one timeframe up"): sub-minute/minute charts overlay 1h,
# hourly charts overlay 1D, daily charts overlay 1W. Evaluated in order; the
# first rule whose upper bound is >= interval_ms wins.
_SUPERIMPOSE_AUTO_TABLE = (
    (3_600_000 - 1,        3_600_000),        # < 1h -> 1h
    (86_400_000 - 1,       86_400_000),        # < 1D -> 1D
    (7 * 86_400_000 - 1,   7 * 86_400_000),    # < 1W -> 1W
)

# Resampling to fewer than this many bars is not useful context (a couple of
# translucent blocks convey nothing) and usually signals the auto-picked TF is
# too coarse for the loaded data range.
_SUPERIMPOSE_MIN_BARS = 3


def _resolve_superimpose_interval(interval_ms: int, n_bars: int,
                                  superimpose) -> int | None:
    """
    Resolve the target interval (ms) for PlotConfig.superimpose, or None if
    superimpose should not be drawn.

    Args:
        interval_ms (int): The chart's own (already finalized) OHLC interval.
        n_bars (int): Number of candles in the finalized ohlc_array - used to
            estimate how many superimposed bars the target interval would
            yield.
        superimpose: PlotConfig.superimpose value. ``False``/``0`` disables;
            ``True`` auto-selects from ``_SUPERIMPOSE_AUTO_TABLE``; an int
            (already parsed from a string by PlotConfig.__post_init__) is
            used as an explicit target interval in ms.

    Returns:
        int | None: Target interval in ms, or None if disabled/unusable.
    """
    import warnings

    if not superimpose or interval_ms <= 0:
        return None

    if superimpose is True:
        target = next(
            (tgt for upper, tgt in _SUPERIMPOSE_AUTO_TABLE if interval_ms <= upper),
            None,
        )
        if target is None:
            warnings.warn(
                f"superimpose=True: no higher timeframe rule covers an "
                f"interval of {interval_ms} ms; skipping superimpose.",
                stacklevel=2,
            )
            return None
    else:
        target = int(superimpose)

    if target <= interval_ms:
        warnings.warn(
            f"superimpose target ({target} ms) must be strictly greater than "
            f"the chart's own interval ({interval_ms} ms); skipping superimpose.",
            stacklevel=2,
        )
        return None

    span_ms = interval_ms * n_bars
    if span_ms < target * _SUPERIMPOSE_MIN_BARS:
        warnings.warn(
            f"superimpose target ({target} ms) would yield fewer than "
            f"{_SUPERIMPOSE_MIN_BARS} bars over the loaded data range; "
            f"skipping superimpose.",
            stacklevel=2,
        )
        return None
