...
# Default hide_on_pan resolution by renderer type (when hide_on_pan is None):
# marker/label-style glyphs (scatter, label, rect, arrow) hide during pan/zoom
# - they are cheap adornments and a brief blank is imperceptible. Continuous
# geometry (line, step, bar, span) stays visible - hiding a bar/line mid-drag
# reads as a glitch; dense bar panels (MACD histogram, etc.) should rely on
# level-of-detail decimation instead of blanking. See PlotConfig.derender_on_pan.
_RENDERER_HIDE_ON_PAN_DEFAULTS: dict[str, bool] = {
    "scatter": True,
    "label":   True,
    "rect":    True,
    "arrow":   True,
...
def resolve_hide_on_pan(renderer: str, hide_on_pan: bool | None) -> bool:
    """
    Resolve the effective hide_on_pan value for a single renderer dimension.

    Args:
        renderer (str): Resolved renderer type for this dimension (e.g. from
            ``IndicatorPlotConfig.dim("renderer", idx)``).
        hide_on_pan (bool | None): Explicit override; None resolves from the
            renderer type via ``_RENDERER_HIDE_ON_PAN_DEFAULTS``.

    Returns:
        bool: Whether this dimension's renderer should be hidden while the
            user pans/wheel-zooms the chart (see ``_configure_derender``).
    """
    if hide_on_pan is not None:
        return hide_on_pan
    return _RENDERER_HIDE_ON_PAN_DEFAULTS.get(renderer, False)
...
class IndicatorPlotConfig:
    ...
    # None (default) -> resolved per dimension from its renderer type via
    # resolve_hide_on_pan() (scatter/label/rect/arrow hide during pan/zoom,
    # line/step/bar/span stay visible). Set explicitly to force a dimension
    # in or out of the global pan/zoom derender (PlotConfig.derender_on_pan)
    # regardless of its renderer - e.g. hide_on_pan=False on a scatter-based
    # signal marker the user wants always visible, or hide_on_pan=True on a
    # bar panel that is unusually dense.
