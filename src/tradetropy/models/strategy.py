...
# The pattern-matching type is imported only for annotations; the runtime
# construction lives in the pattern methods, which import it lazily.
if TYPE_CHECKING:
...
class Strategy:
    ...
    def add_pattern_matcher(
        ...
        *,
        ...
        Active window
            A standalone ``@active_bars(end)`` or ``@active_bars(start, end)``
            directive line bounds how long a completed match stays reported by
            ``.last``. The age is counted in OHLC bars since the CONFIRMATION
            bar of the last matched pivot (age 0 == confirmation bar; the bars
            needed to confirm the pivot do NOT consume the window). Outside
            ``start <= age <= end`` the proxy returns None. Example: a match
            available only for the 10 bars after its confirmation::
