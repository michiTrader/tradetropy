...
def _run_tick_mode_worker(strategy, shared_store, ohlc_stores, opened_shm,
    ...
    # Map each pattern matcher to its source OHLC store so the proxy converts
    # the tick cursor to a closed-candle index (same as the single-engine
    # backtest). Needed both for causal last_match_at and for the @active_bars
    # activation window, which measures bars since the last pivot's confirmation.
    _matcher_ohlc_store: dict = {}
    if strategy._pattern_matcher_defs:
        from tradetropy.ta.pattern._builder import iter_pattern_matcher_defs

        def _def_for_proxy(proxy):
            for d in strategy._indicator_defs:
                if d["proxy"] is proxy:
                    return d
            return None

        for pm_def, _, ohlc_proxy, *_rest in iter_pattern_matcher_defs(
            strategy, _def_for_proxy
        ):
            st = ohlc_stores.get(id(ohlc_proxy))
            if st is not None:
                _matcher_ohlc_store[id(pm_def)] = st

    for pm_def in strategy._pattern_matcher_defs:
        pm_key = id(pm_def.pattern)
        store  = shared_store.pattern_stores.get(pm_key)
        if store is not None:
            pm_def.proxy._connect_backtest(
                store, ohlc_store=_matcher_ohlc_store.get(id(pm_def))
            )
