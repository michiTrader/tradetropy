...
def compute_stats_fast(
    ...
    *,
    ...
    # ── Trades to arrays ──────────────────────────────────────────────────────
    (entry_ms, exit_ms, size, entry_px, pnl_net, direction, commission,
     ticket, position_id) = _trade_arrays(trades)
    ...
    n_positions, pos_longs, pos_shorts = _count_positions_by_direction(
        position_id[order], ticket[order], c_dir,
    )
    ...
    metrics: "OrderedDict[str, Any]" = OrderedDict([
        ...
        ("# Positions Long", pos_longs),
        ("# Positions Short", pos_shorts),
...
def _position_group_keys(position_id: np.ndarray, ticket: np.ndarray) -> np.ndarray:
    """
    Resolve the grouping key used to aggregate closed trades into positions.

    Mirrors ``stats.py::_position_group_key``: falls back to the trade's own
    ``ticket`` when ``position_id`` is missing/zero (connectors that do not
    track a real position id), so # Positions never silently under-counts.

    Args:
        position_id (np.ndarray): position_id per closed trade
        ticket (np.ndarray): ticket per closed trade

    Returns:
        np.ndarray: grouping key, one value per row
    """
    return np.where(position_id != 0, position_id, ticket)


def _count_positions_by_direction(
    position_id: np.ndarray, ticket: np.ndarray, direction: np.ndarray,
) -> tuple[int, int, int]:
    """
    Count distinct closed positions, overall and by direction.

    A position's direction is taken from its first occurrence, matching
    ``stats.py::count_positions_by_direction``'s groupby-first semantics.

    Args:
        position_id (np.ndarray): position_id per closed trade
        ticket (np.ndarray): ticket per closed trade
        direction (np.ndarray): 1 (long) / -1 (short) per closed trade

    Returns:
        tuple: (n_positions, longs, shorts)
    """
    if position_id.size == 0:
        return 0, 0, 0
    key = _position_group_keys(position_id, ticket)
    uniq_keys, first_idx = np.unique(key, return_index=True)
    first_dir = direction[first_idx]
    n_positions = int(uniq_keys.size)
    longs = int(np.sum(first_dir == 1))
    shorts = int(np.sum(first_dir == -1))
    return n_positions, longs, shorts
...
def _trade_arrays(trades: list):
    ...
    ticket = np.zeros(n, dtype=np.int64)
    position_id = np.zeros(n, dtype=np.int64)
    for i, t in enumerate(trades):
        ...
        ticket[i] = int(getattr(t, "ticket", 0) or 0)
        position_id[i] = int(getattr(t, "position_id", 0) or 0)
